...
```json
{
  ...
  "context": { "pct_used": 18, "used_tokens": 36000, "limit_tokens": 200000 },
  "provider": "anthropic"
}
```

`pr` is present only when an open pull request is detected for the current branch; `context` is present after the first response when the model's context limit is known. `provider` and `context` are Ante extensions. The command also receives `COLUMNS` and `LINES` (current terminal size) and `ANTE_PROJECT_DIR` (nearest ancestor with `.git`) in its environment.
