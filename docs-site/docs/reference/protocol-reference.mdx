...
| `policy` | `PermissionMode?` | Tool approval policy: `"default"`, `"auto"`, or `"yolo"`. `null` uses default |
...
Token usage statistics for the session. `context` is omitted before the first response or when the model context limit is unknown.
...
```json
{
  "event": {
    "UsageUpdate": {
      ...
      "context": { "used_tokens": 1800, "limit_tokens": 200000, "pct_left": 99 }
    }
  }
}
```
