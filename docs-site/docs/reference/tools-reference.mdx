...
Search the web and return results with citations. Requires approval. Not in the default tool set — enable with `--allowed-tools`. Uses the provider's native web search, so it only works on a provider that advertises `supports_web_search` (Anthropic, OpenAI, Gemini, Grok, OpenRouter, Antix, and Ali Coding Plan); on other providers the call returns an error.

## Opt-in tools

These are not enabled by default. Turn them on with `--allowed-tools`, or use `--simple-agent` for an agent set (`PTY`, `ViewImage`, `WebSearch`) tuned around interactive terminal work.
...
`WebSearch`, `ViewImage`, and `PTY` are opt-in.
