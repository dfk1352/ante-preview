...
It needs no TTY and no API key: MCP warm-up is skipped and the session reaches readiness from the catalog alone. The report lists three checks — `catalog` (provider/model counts), `session` (the resolved `provider/model`), and `first frame` (the headless render) — followed by a total. Warnings (such as no authenticated provider, or a skipped catalog entry) are listed below the table without failing the check. `doctor` exits non-zero if any hard check fails, so it works in scripts.
