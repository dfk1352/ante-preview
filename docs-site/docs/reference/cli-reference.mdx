...
| `--include-tools <TOOLS>...` | Only allow these tools. Values are space-separated. (`--allowed-tools` and `--tools` are accepted as aliases.) |
| `--exclude-tools <TOOLS>...` | Remove these tools from the session. Values are space-separated. (`--disallowed-tools` is accepted as an alias.) |
...
| `--simple-agent` | Use the built-in simple agent prompt with `PTY`, `ViewImage`, and `WebSearch` tools. |
