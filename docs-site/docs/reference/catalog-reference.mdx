...
The `openai-subscription` provider (ChatGPT/Codex OAuth) serves only `gpt-5.5` — the other GPT-5 ids aren't available on ChatGPT accounts.
...
| `supports_web_search` | — | Whether the provider exposes a native web-search affordance. Gates whether the `WebSearch` tool works on a session using this provider (defaults to `false`). |
