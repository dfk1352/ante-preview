...
| `--include-tools <TOOLS>...` | Only allow these tools (space-separated). Alias: `--allowed-tools` |
| `--exclude-tools <TOOLS>...` | Disallow these tools (space-separated). Alias: `--disallowed-tools` |
...
| `--simple-agent` | Simple agent mode — uses a minimal system prompt with `PTY`, `ViewImage`, and `WebSearch` tools |
...
The `--simple-agent` flag starts Ante with a minimal system prompt and a reduced tool set (`PTY`, `ViewImage`, `WebSearch`). This is useful for lightweight automation where you don't need the full coding agent:

```bash
ante --simple-agent -p "take a screenshot and describe what you see"
```

If you also pass `--system-prompt` or `--include-tools`, those take precedence over the simple-agent defaults.
